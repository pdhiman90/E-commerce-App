            ...
            </ul>
            {
                products.length > 0 ? products.map((item, index) =>
                    <ul className="Table" key={item._id}>
                        ...
                        <li>{item.category}</li>
                        <li>
                            <button onClick={() => deleteProduct(item._id)}><i className="bin-trash">Delete</i></button>
                            {/* <button type="button" class="btn btn-danger dltbtn" onClick={()=>deleteProduct(item._id)}>Delete</button> */}
